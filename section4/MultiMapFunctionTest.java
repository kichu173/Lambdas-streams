...
public class MultiMapFunctionTest {
	...
	private static void afterLambdas(List<String> cities) {
		...
		for (String city : cities) {
			...
			// computeIfAbsent- checks the first parameter, if absent it executes the second parameter which is functional interface, if present it simply ignores second parameter and it retrieves the list associated to the first parameter, add the city.
			// fn interface which initializes the new arraylist and add the current iteration city to that array list
			// {b, [bengaluru]}
